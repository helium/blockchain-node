...
init([]) ->
    ...
    {ok, {SupFlags,
      [
       ...
       ?WORKER(txn_follower,
               blockchain_follower, [[{follower_module, {bn_txns, [{base_dir, BaseDir}]}}]]),
       ?WORKER(pending_txn_follower,
               blockchain_follower, [[{follower_module, {bn_pending_txns, [{base_dir, BaseDir}]}}]]),
       ?WORKER(bn_wallets, [[{base_dir, BaseDir}]]),
       ?WORKER(elli, [ [{callback, bn_jsonrpc_handler},
                        {port, NodePort}
                       ] ])
      ]}
    }.
