...
-behaviour(helium_follower_service_bhvr).
...
%% -------------------------------------------------------------------
%% helium_follower_service_bhvr callback functions
%% -------------------------------------------------------------------
...
handle_info({blockchain_event, {add_block, BlockHash, _Sync, _Ledger}}, StreamState) ->
    ...
    case blockchain:get_block(BlockHash, Chain) of
        {ok, Block} ->
            ...
            NewStreamState = send_txn_sequence(SortedTxns, Height, EpochStart, SigFun, Chain, StreamState);
        _ ->
...
txn_stream(
    ...
    case process_previous_blocks(Height, Hash, SigFun, Chain, StreamState) of
        {ok, StreamState1} ->
            HandlerState = grpcbox_stream:stream_handler_start(StreamState1),
            ...
            case blockchain:get_block(Height, Chain) of
                {ok, PriorBlock} ->
                    ...
                    StreamState2 = lists:foldl(fun(HeightX, StateAcc) ->
                                                   ...
                                                   NewStateAcc = send_txn_sequence(SortedTxnsX, HeightX, EpochX, SigFun, Chain, StateAcc)
...
send_txn_sequence(SortedTxns, Height, Epoch, SigFun, Chain, StreamState) ->
    lists:foldl(fun(Txn, StateAcc) ->
                    Msg = encode_follower_resp(Txn, Height, Epoch, SigFun, Chain),
